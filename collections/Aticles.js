...
Globals.schemas.articleSchema =  new SimpleSchema({
   title: {
       type: String,
       label: 'Titre',
       max: 200
    ...
    noModif: {// Permet de savoir si un article a déjà été publié une fois ou non (Permet de définir s'il faut envoyer ou non des notifications)
        ...
        autoValue: function () {
            if (this.isInsert){
                ...
                var noModif = Articles.findOne(this.docId, {fileds: {noModif:1}}).noModif;
                if(noModif === undifined){
                    noModif = true;
                }
...
Articles.attachSchema(Globals.schemas.articleSchema);
